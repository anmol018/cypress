...
describe('Analytics Report', function () {		
 ...
 //Roll Rate Analysis
  cy.get('.ng-isolate-scope > .wid-100 > .mr10').as('scenario').find('option').each(($RRAoption) => {
  cy.get('.ng-isolate-scope > .wid-100 > .mr10').select(($RRAoption).text())
  cy.get('.ng-isolate-scope > .wid-100 > [ng-if="vm.dropDown.length"]').as('dropdown').find('option').each(($months) => {
...
//Trend Analysis
  ...
  cy.log('Receivables Concentrations',($TAscenario).text())
})
})
})
...
//Trend Analysis
    cy.get('.ml5 chart [ng-model="vm.period"]').find('option').each(($TAscenario) => {
    cy.get('.ml5 chart [ng-model="vm.period"]').select(($TAscenario).text()).wait('@iteration').then((response) => {

...
describe('Analytics Report', function () {		
    ...
    it('Post Login, Analytics Report, Strats', function(){	
        cy.server()
    ...
    //Strats
    cy.get('strats select').should('be.visible').find('option').each(($Soption) => {
    cy.get('strats select').select(($Soption).text()).wait('@iteration').then((response) => {
        expect(response.status).to.eq(200)
        cy.timeConversion(response.duration).then(($value) =>{
    cy.log('Response Duration:',$value)
    cy.log('Strats',($Soption).text())
})
})
})

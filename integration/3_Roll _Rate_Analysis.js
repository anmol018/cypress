...
describe('Analytics Report', function () {		
 ...
 //Roll Rate Analysis
  cy.get('roll-rate [ng-model="vm.supportField"]').find('option').each(($RRAoption) => {
  cy.get('roll-rate [ng-model="vm.supportField"]').select(($RRAoption).text())
  cy.get('roll-rate [ng-model="vm.date"]').find('option').each(($months) => {
  cy.get('roll-rate [ng-model="vm.date"]').select(($months).text()).wait('@iteration').then((response) => {

...
describe('Analytics Report', function () {		
    ...
    it('Post Login, Analytics Report, Strats-Chart', function(){	
        cy.server()
    ...
    //Strats-Chart
    cy.get('.mr5 > loss-rate [ng-model="vm.supportPeriod"]').should('be.visible').find('option').each(($SCoption) => {
    cy.get('.mr5 > loss-rate [ng-model="vm.supportPeriod"]').select(($SCoption).text())
    cy.get('.mr5 > loss-rate [ng-model="vm.supportLimit"]').find('option').each(($SCscenario)=>{
    cy.get('.mr5 > loss-rate [ng-model="vm.supportLimit"]').select(($SCscenario).text()).wait('@iteration').then((response) => {
        expect(response.status).to.eq(200)
        cy.timeConversion(response.duration).then(($value) =>{
    cy.log('Response Duration:',$value)
    cy.log(($SCoption).text(),($SCscenario).text())

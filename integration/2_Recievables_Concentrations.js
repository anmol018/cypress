...
describe('Analytics Report', function () {		
    ...
    //Receivables Concentrations centrations centrations div.wid-50.fl.pr5 > chart > div > div > select
    cy.get('.pr5 chart [ng-model="vm.period"]').find('option').each(($RCscenario) => {
    cy.get('.pr5 chart [ng-model="vm.period"]').select(($RCscenario).text()).wait('@iteration').then((response) => {
   ...
   //Receivables Concentrations
    cy.get('.pl5> chart [ng-model="vm.period"]').find('option').each(($RCscenario1) => {
    cy.get('.pl5> chart [ng-model="vm.period"]').select(($RCscenario1).text()).wait('@iteration').then((response) => {
        expect(response.status).to.eq(200)
        cy.timeConversion(response.duration).then(($value) =>{
        cy.log('Response Duration:',$value)

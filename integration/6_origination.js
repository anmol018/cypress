...
describe('Analytics Report', function () {		
    ...
    it('Post Login, Analytics Report, Origination', function(){	
        cy.server()
 ...
 //Loss Rate Analysis
 cy.get('#portfolioOverviewMix [ng-model="vm.supportPeriod"]').should('be.visible').find('option').each(($Ooption)=>{
     cy.get('#portfolioOverviewMix [ng-model="vm.supportPeriod"]').select(($Ooption).text()).wait('@iteration').then((response) => {
        expect(response.status).to.eq(200)
        cy.timeConversion(response.duration).then(($value) =>{
        cy.log('Response Duration:',$value)
     cy.log('Origination',($Ooption).text())
 })
})
})

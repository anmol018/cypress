/// <reference types="Cypress" />
//Cypress.config("defaultCommandTimeout", 10)
const addContext = require('mochawesome/addContext')
describe('Analytics Report', function () {		
    beforeEach(function(){
        cy.viewport(1366, 768)
        cy.loginByCSRF()
         .then((resp) => {
         expect(resp.status).to.eq(204)
         cy.log("Login Successful")
    })
    })
    it('Post Login, Analytics Report, Loss Rate Analysis', function(){	
      cy.server()
        cy.route('POST','/api/getViewExecutionResult?t=7').as('iteration')
    cy.visit('/api/custom-report/view/run-report/39?t=7++++++++++++&timestampActuals=%7B%22id%22%3A324%2C%22date%22%3A%222019-08-01%22%2C%22templateStatus%22%3A%22APPROVED%22%2C%22template%22%3A%7B%22id%22%3A0%2C%22name%22%3Anull%7D%2C%22status%22%3A%22ADDED%22%2C%22source%22%3A%22ACTUALS%22%2C%22freeze%22%3Afalse%7D&entity=PORTFOLIO++++++++++++&entityId=undefined&rId=39');
 //Loss Rate Analysis
  cy.get('#lossRateAnalysis [ng-model="vm.supportPeriod"]').should('be.visible').find('option').each(($LRAoption) => {
  cy.get('#lossRateAnalysis [ng-model="vm.supportPeriod"]').select(($LRAoption).text())
  cy.get('.fl> loss-rate [ng-model="vm.supportLimit"]').find('option').each(($LRAscenario) => {
  cy.get('.fl> loss-rate [ng-model="vm.supportLimit"]').select(($LRAscenario).text()).wait('@iteration').then((response) => {
    expect(response.status).to.eq(200)
    cy.timeConversion(response.duration).then(($value) =>{
    cy.log('Response Duration:',$value)
  //cy.get('#lossRateAnalysis > .ptb5 > .ng-isolate-scope > .plr10 > .icon-arrowdown').click()
  //cy.get('#lossRateAnalysis > .ptb5 > .ng-isolate-scope > .p5').find('label').each(($country)=> {
  //cy.get('#lossRateAnalysis > .ptb5 > .ng-isolate-scope > .p5 > .noStyle .ng-pristine').click(($country).text())    
  cy.log(($LRAoption).text(),($LRAscenario).text())
})
})
})
})
Cypress.on('test:after:run', (test, runnable) => {
  if (test.state === 'failed') {
      const screenshotFileName = `${runnable.parent.title} -- ${test.title} (failed).png`
      addContext({ test }, `${Cypress.spec.name}/${screenshotFileName}`)
  }
  })
})
})